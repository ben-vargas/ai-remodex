...
extension CodexService {
    ...
    func flushAllPendingStreamingDeltas() {
        for key in Array(pendingAssistantDeltasByKey.keys) {
            flushPendingAssistantDeltas(forKey: key)
        }
    ...
    func cancelPendingStreamingDeltaFlushes(for threadId: String) {
        let assistantKeys = pendingAssistantDeltasByKey
            .filter { $0.value.threadId == threadId }
            .map(\.key)
        for key in assistantKeys {
            assistantDeltaFlushTasksByKey[key]?.cancel()
            assistantDeltaFlushTasksByKey.removeValue(forKey: key)
            pendingAssistantDeltasByKey.removeValue(forKey: key)
        }
    ...
    func cancelAllPendingStreamingDeltaFlushes() {
        assistantDeltaFlushTasksByKey.values.forEach { $0.cancel() }
        assistantDeltaFlushTasksByKey.removeAll()
        pendingAssistantDeltasByKey.removeAll()
    ...
    // Coalesces high-frequency assistant tokens into one UI mutation per display frame.
    func enqueueAssistantDelta(threadId: String, turnId: String, itemId: String?, delta: String) {
        guard !delta.isEmpty else { return }

        let key = assistantDeltaBufferKey(threadId: threadId, turnId: turnId, itemId: itemId)
        if pendingAssistantDeltasByKey[key] == nil {
            pendingAssistantDeltasByKey[key] = PendingAssistantStreamingDeltas(
                threadId: threadId,
                turnId: turnId,
                itemId: itemId,
                deltas: []
        ...
        pendingAssistantDeltasByKey[key]?.deltas.append(delta)

        guard assistantDeltaFlushTasksByKey[key] == nil else { return }
        assistantDeltaFlushTasksByKey[key] = Task { @MainActor [weak self] in
            try? await Task.sleep(nanoseconds: StreamingDeltaCoalescingPolicy.flushDelayNanoseconds)
            guard !Task.isCancelled else { return }
            self?.flushPendingAssistantDeltas(forKey: key)
        }
    }
    ...
    func appendAssistantDelta(threadId: String, turnId: String, itemId: String?, delta: String) {
        applyAssistantDeltas(threadId: threadId, turnId: turnId, itemId: itemId, deltas: [delta])
    }

    private func applyAssistantDeltas(threadId: String, turnId: String, itemId: String?, deltas: [String]) {
        let nonEmptyDeltas = deltas.filter { !$0.isEmpty }
        guard !nonEmptyDeltas.isEmpty else {
            return
        }
        ...
        let nextText = nonEmptyDeltas.reduce(currentText) { current, delta in
            mergeAssistantDelta(existingText: current, incomingDelta: delta)
        }
    ...
    func flushPendingAssistantDeltas(threadId: String, turnId: String?, itemId: String?) {
        guard let turnId else { return }
        flushPendingAssistantDeltas(
            forKey: assistantDeltaBufferKey(threadId: threadId, turnId: turnId, itemId: itemId)
        )
    }

    func flushPendingAssistantDeltasForTurn(threadId: String, turnId: String?) {
        guard let turnId else { return }
        let keys = pendingAssistantDeltasByKey
    ...
    private func flushPendingAssistantDeltas(forKey key: String) {
        assistantDeltaFlushTasksByKey[key]?.cancel()
        assistantDeltaFlushTasksByKey.removeValue(forKey: key)
        guard let pending = pendingAssistantDeltasByKey.removeValue(forKey: key) else {
            return
        }
    ...
    private func assistantDeltaBufferKey(threadId: String, turnId: String, itemId: String?) -> String {
        if let normalizedItemId = normalizedStreamingItemID(itemId) {
            return assistantStreamingMessageKey(threadId: threadId, turnId: turnId, itemId: normalizedItemId)
        }
        return streamingMessageKey(threadId: threadId, turnId: turnId)
    }
    ...
    func completeAssistantMessage(threadId: String, turnId: String?, itemId: String?, text: String) {
        ...
        let resolvedTurnId = turnId ?? activeTurnIdByThread[threadId]
        flushPendingAssistantDeltasForTurn(threadId: threadId, turnId: resolvedTurnId)
        let now = Date()
